...
} from "../workflows/send-abandoned-carts/send-abandoned-carts";
...
export default async function abandonedCartJob(container: MedusaContainer) {
  ...
  do {
    const { data: abandonedCarts, metadata } = await query.graph({
      entity: "cart",
      fields: ["id", "email", "items.*", "metadata", "customer.*"],
      filters: {
        updated_at: {
          $lt: oneDayAgo,
        },
    ...
    const cartsWithItems = abandonedCarts.filter(
      (cart) => cart.items?.length > 0 && !cart.metadata?.abandoned_notification
    );
